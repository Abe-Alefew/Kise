...
void main() {
  ...
  group('DioClient.createBaseDio', () {
    ...
    setUp(() {
      dio = DioClient.createBaseDio();
    });

    group('base URL', () {
      test('uses ApiEndpoints.baseUrl', () {
        expect(dio.options.baseUrl, ApiEndpoints.baseUrl);
      });

      test('default base URL contains expected host', () {
        expect(dio.options.baseUrl, contains('127.0.0.1:3000'));
      });
    });

    group('timeouts', () {
      test('connectTimeout defaults to 15 seconds', () {
        expect(dio.options.connectTimeout, const Duration(seconds: 15));
      });

      test('receiveTimeout defaults to 20 seconds', () {
        expect(dio.options.receiveTimeout, const Duration(seconds: 20));
      });

      test('sendTimeout defaults to 20 seconds', () {
        expect(dio.options.sendTimeout, const Duration(seconds: 20));
      });

      test('custom timeouts are applied when provided', () {
        final customDio = DioClient.createBaseDio(
          connectTimeout: const Duration(seconds: 5),
          receiveTimeout: const Duration(seconds: 10),
          sendTimeout: const Duration(seconds: 10),
        );
        expect(customDio.options.connectTimeout, const Duration(seconds: 5));
        expect(customDio.options.receiveTimeout, const Duration(seconds: 10));
      });
    });
    ...
    group('validateStatus', () {
      test('accepts status codes below 500', () {
      ...
      test('rejects status 500 and above', () {
        final validate = dio.options.validateStatus;
        expect(validate(500), isFalse);
        expect(validate(502), isFalse);
    ...
    group('interceptors', () {
      test('no custom app interceptors are added by createBaseDio', () {
        // Dio adds ImplyContentTypeInterceptor internally; createBaseDio adds none
        // on top of that — app interceptors (AuthInterceptor) are wired externally.
        final appInterceptors = dio.interceptors
            .whereType<QueuedInterceptor>()
            .toList();
        expect(appInterceptors, isEmpty);
      });
    });
  });
  ...
  group('ApiEndpoints', () {
    ...
    test('authLogin path is "/auth/login"', () {
      expect(ApiEndpoints.authLogin, '/auth/login');
    });

    test('authRegister path is "/auth/register"', () {
      expect(ApiEndpoints.authRegister, '/auth/register');
    });

    test('transactions path is "/transactions"', () {
      expect(ApiEndpoints.transactions, '/transactions');
    });

    test('goals path is "/goals"', () {
      expect(ApiEndpoints.goals, '/goals');
    });

    test('debts path is "/debts"', () {
      expect(ApiEndpoints.debts, '/debts');
    });

    test('dashboardHome path is "/dashboard/home"', () {
      expect(ApiEndpoints.dashboardHome, '/dashboard/home');
    });
  });
}
